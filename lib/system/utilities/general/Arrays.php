...
class Arrays{
	...
	static function stringArray($var){
		if(!is_array($var)){
			return explode(',',$var);
		}
		return $var;
	...
	*/
	...
	@param	array	the array to be modified
	*/
	static function remove(&$array,$value = false){
	...
	/// compile into an array the values of the keys of multiple arrays
	/**
	...
	*/
	function compileKey($array,$key = 'id',$name=null){
		if(is_array($array)){
			$newArray = array();
	...
	static function &extract($keys,$extactee,&$extractTo=null){
	...
	/**
		@param	map	array	array(extractTo key name => extractee key name)
	*/
	static function &map($map,$extractee,&$extractTo=null){
		if(!is_array($extractTo)){
			$extractTo = array();
		}
