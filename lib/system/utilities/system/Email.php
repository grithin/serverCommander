...
Use with just HTML message
	email::send(html,to,subject,from);
Use with attachments:
	$message = '<h2>Hello</h2><p>Bob</p>';
	$email = new Email($message);
	$email->attach('/var/www/humval/imgs/crystal.png');
	email::send('spamme@capob.com','testing',$email,'spamme@capob.com');
Use in a chained way
	i()->Email("bob sucks")->attach('/var/log/test.log')->send('spamme@capob.com','The state of bob','spamme@capob.com');
@endverbatim
*/
class Email{
	...
	static function __callStatic($name,$arguments){
		//assumes first argument to be Email object or argument to Email constructor
